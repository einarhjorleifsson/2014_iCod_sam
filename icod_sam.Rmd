...
## Assessment
```{r, eval=FALSE}
# sam   <-  FLSAM(stk,tun,ctr,run.dir = dir.run)
```
...
```
This occurs quite late in the function code within the *SAM2FLR*-function when reading the mortality output matrix. What has been done here is to split the *FLSAM* function into two pieces. Lets source them:
```{r}
...
```{r, warning=FALSE}
res <- residuals(sam)
res$fleet <- ifelse(res$fleet %in% "Fleet 1","catch","smb")
