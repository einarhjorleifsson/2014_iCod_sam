FLSAM_run <-function(stck,tun,ctrl,run.dir=tempdir(),batch.mode=FALSE,pin.sam=NULL) {
  #---------------------------------------------------
  ...
  # EINAR - OUTCOMMENTED: res <- SAM2FLR(ctrl=ctrl,run.dir=run.dir)
  ...
  # EINAR - OUTCOMMENTED:  levels(res@residuals$fleet) <- names(ctrl@fleets)
  # EINAR - OUTCOMMENTED:  return(res)
}
...
SAM2FLR_fix <-function(ctrl,admb.stem="sam",run.dir=tempdir()) {
  ...
  res@harvest <- flq
  res@harvest@units <- "f"
  # EINAR - OUTCOMMENTED: for(a in  colnames(res@states)){
  # EINAR - OUTCOMMENTED:   states.key <- res@states["catch",a] 
  # EINAR - OUTCOMMENTED:   res@harvest[a,] <- exp(f.stateEst[states.key,])
  # EINAR - OUTCOMMENTED: }
  # EINAR QUICK FIX
  f.stateEst <- stateEst[-c(1:n.ages),,drop=FALSE]
  harvest(res)[3:10] <- exp(f.stateEst)
  harvest(res)[1:2] <- 0
  harvest(res)[11:14] <- harvest(res)[10]
